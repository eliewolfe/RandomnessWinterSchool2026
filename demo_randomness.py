"""Randomness-focused end-to-end demos with unclustered preparations (|A|=1)."""
...
def main() -> None:
    ...
    preparation_indices_example_4 = [(idx,) for idx in range(len(labels))]
    measurement_indices_example_4 = [tuple(label_to_index[ch] for ch in context) for context in contexts]
    ...
    preparation_indices_example_5 = [(idx,) for idx in range(len(peres_kets))]
    measurement_indices_example_5 = [tuple(range(4 * y, 4 * (y + 1))) for y in range(6)]
