"""Pedagogical QKD demo: qubit (X+Z)/(X-Z) with clustered preparations.

Recommended execution:
    python -m contextualityqkd.demos.qkd_qubit_xplusz_xminusz
"""
...
def main() -> None:
    ...
    ContextualityScenario.print_title("QKD: (X+Z) and (X-Z) measurements")

    # ---------------------------------------------------------------------
    # 1) Build the required state/effect kets.
    #    States are still |0>,|1>,|+>,|->, while measurements are now:
    #      - (X+Z) basis
    #      - (X-Z) basis
    # ---------------------------------------------------------------------
    ket0 = xz_plane_ket(0)
    ...
    # QKD clustering: two preparation settings, each with two source outcomes.
    ...
    # ---------------------------------------------------------------------
    # 3) Construct the scenario object used by all LP/analysis routines.
    # ---------------------------------------------------------------------
    ...
    # ---------------------------------------------------------------------
    # 4) Print symbolic objects and the main randomness/contextuality outputs.
    # ---------------------------------------------------------------------
